...
let user = {name: 'Albina'};
let javaScript = {html: 'JavaScript'};

function countLetterA(param) {
	return param.split("a").length-1;
}
 ...
 * */
function reverseEachWord(str, param) {
	if(param === true) {
		return str.split("").reverse().join("").split(" ").reverse().join(" ");
	}else{
		return str;
...
console.log(reverseEachWord('Hi my Name is', true));
// iH ym emaN si
...
var sentence = 'url http url www url http';
